...
export default class IanasciidocpreviewActiveEditorInfoView {
  ...
  // #################   LOK AT THIS #########################
    ...
    let htmloutputDir = path.join(__dirname, 'tmp');
    try {
    ...
    let stylesDir = path.join(htmloutputDir, 'css');
    try {
    ...
    function getAsciidoctorCssPath() {
      ...
      let htmloutputDir = path.join(__dirname, 'tmp');
      let asciidoctorCssPath = path.join(stylesDir, 'asciidoctor.css');
      if (!fs.existsSync(asciidoctorCssPath)) {
        const file = fs.createWriteStream(asciidoctorCssPath);
        const request = https.get("https://raw.githubusercontent.com/asciidoctor/asciidoctor/v2.0.x/src/stylesheets/asciidoctor.css", function(response) {
           response.pipe(file);
    ...
    function getTextOpenWrite() {   // function getTextOpenWrite()
      ...
      let transformscaleDir = htmloutputDir
      let htmloutputFile = path.join(htmloutputDir, 'iansasciidocpreviewhtmloutput.html');
      let htmloutputFileRawFromAsciidoc = path.join(htmloutputDir, 'htmloutputFileRawFromAsciidoc.html');
      let transformScaleFile = path.join(htmloutputDir, 'iansasciidocpreviewtransformscale.txt');
      console.log('htmloutputDir=\n' + htmloutputDir);
      console.log("transformscaleDir=\n" + transformscaleDir);
      ...
      fs.writeFileSync(path.join(htmloutputDir, "XXXasciidocInputFromEditor_words"), asciidocInputFromEditor_words);  // file with original .adoc which is in string words the original asciidoc file - just to check

      // write out raw file created by asciidoctor.js that can be read by browser
      fs.writeFileSync(htmloutputFileRawFromAsciidoc, htmloutput);
      ...
      let asciidocUrlHref = '<a href="' + htmloutputDir + '" >' +    asciidocUrl + '</a>'; console.log('asciidocUrlHref =\n' + asciidocUrlHref);
      ...
      try {
        transformScale = fs.readFileSync(transformScaleFile, {encoding:'utf8' });
        ...
        if (err.code === 'ENOENT') {  //file does not exist
          console.log( 'File not found: Creating...');  // create wi default
          fs.writeFileSync(transformScaleFile, String(zoomScaleDefault.toFixed(2)));
        }
      };
      ...
      /////////////////////////////////////////////////     scrollToHere   //////////////////////////////////////////
          let scrollToHereFile = path.join(htmloutputDir, 'iansasciidocpreviewscrolltohere.txt');
          ...
          // read zoom transform factor asynchronously cf Flannagan p605
              //fs.readFileSync(htmloutputDir + htmloutputFile, newhtmloutput);
          try {
            scrollToHere = fs.readFileSync(scrollToHereFile, {encoding:'utf8' });
            ...
            if (err.code === 'ENOENT') {  //file does not exist
              ...
              fs.writeFileSync(scrollToHereFile, String(scrollToHereDefault.toFixed(2)));
            }
          };
          alert( 'after try/catch: scrollToHereFile found:' + scrollToHereFile + " value: " + scrollToHere);
     ...
     `      <script>
       ...
       fs.writeFileSync(htmloutputFile, newhtmloutput);
