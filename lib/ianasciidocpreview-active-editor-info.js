...
import { htmloutputDir } from './ianasciidocpreview-active-editor-info-view'; // NB {  } essential
...
//                       ###############################################
zoomInOutWiTextFile(zoomStep= +0.20) {   // default +0.20 is this a good idea
...
/*  ORIGINAL transformsScaleUp  before tryCatch added
  ...
  transformScaleUp() {
    ...
    console.log("l192 ...info.js  htmloutputDir=" + htmloutputDir);

    try{
      XXXtransformScale = fs.readFileSync(htmloutputDir + 'temp/iansasciidocpreviewtransformscale.txt');
      XXXtransformScale = (Number(XXXtransformScale)).toFixed(2);
      console.log("XVCT XXXtransformScale 1 =" + XXXtransformScale + "  typeof " + typeof(XXXtransformScale) );
      XXXtransformScale = 0.20 + Number(XXXtransformScale) ;
      fs.writeFileSync(htmloutputDir + 'temp/iansasciidocpreviewtransformscale.txt', String((Number(XXXtransformScale)).toFixed(2)));
      ...
      if (err) {
        fs.writeFileSync(htmloutputDir + 'temp/iansasciidocpreviewtransformscale.txt', String(1.00));
    ...
    // https://stackoverflow.com/questions/56203800/atom-package-create-a-file
    //Essential to do this or new transformScale cannot be read from file  I cant remember why.
    atom.workspace.open(htmloutputDir + 'temp/iansasciidocpreviewtransformscale.txt')
      .then( newTab => {
        newTab.insertText(' ');
        newTab.save();
        newTab.destroy();
  ...
  */
  ...
  openBrowserPopup() {   // called wi Ctrl-Alt-F  NB C-A-B is used by JavaScript
       ...
       // https://stackoverflow.com/questions/5588658/auto-reload-browser-when-i-save-changes-to-html-file-in-chrome
        ...
        cp.exec("/usr/bin/falkon " + htmloutputDir + "temp/iansasciidocpreviewhtmloutput.html", options, (e, stdout, stderr) => {
          if (e) throw e
        ...
        //navigator.clipboard.writeText("file:///media/AcerWinData/github_from_home/CloneOfianasciidocpreview-active-editor-info-20230915_ChromeZoomWiTransformFactorInFile/temp/iansasciidocpreviewhtmloutput.html").then(
        navigator.clipboard.writeText(htmloutputDir + "temp/iansasciidocpreviewhtmloutput.html").then(
          () => {
